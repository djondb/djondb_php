...
class BSONObj {
	...
	function compare($other) {
		return BSONObj_compare($this->_cPtr,$other);
	}

	function remove($kkey) {
		BSONObj_remove($this->_cPtr,$kkey);
	}
}

class BSONParseException {
	public $_cPtr=null;
	protected $_pData=array();
	...
	function __construct($code_or_orig,$error=null) {
		if (is_resource($code_or_orig) && get_resource_type($code_or_orig) === '_p_BSONParseException') {
			$this->_cPtr=$code_or_orig;
			return;
		}
		switch (func_num_args()) {
		case 1: $this->_cPtr=new_BSONParseException($code_or_orig); break;
		default: $this->_cPtr=new_BSONParseException($code_or_orig,$error);
		}
	}

	function what() {
		return BSONParseException_what($this->_cPtr);
	}

	function errorCode() {
		return BSONParseException_errorCode($this->_cPtr);
	}
}
...
class DjondbConnection {
	...
	function createIndex($indexDef) {
		DjondbConnection_createIndex($this->_cPtr,$indexDef);
	}
	...
	function backup($db,$destFile,$options=null) {
		switch (func_num_args()) {
		case 2: $r=DjondbConnection_backup($this->_cPtr,$db,$destFile); break;
		default: $r=DjondbConnection_backup($this->_cPtr,$db,$destFile,$options);
		}
		return $r;
	}
}
...
class ConnectionReference {
	...
	function __isset($var) {
		if (function_exists('ConnectionReference_'.$var.'_get')) return true;
		if ($var === 'thisown') return true;
		return array_key_exists($var, $this->_pData);
	}
